...
`MockLLMChatAdapter` extends `BaseBackendAdapter` with the mock-llm engine. Combined with the mock demo server (`examples/demo/server-mock.ts`), it provides a zero-auth HTTP chat server for E2E tests.
...
npx tsx examples/demo/server-mock.ts
