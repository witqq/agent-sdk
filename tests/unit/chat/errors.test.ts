...
describe("ChatErrorCode", () => {
  it("has all 20 codes", () => {
    const codes = Object.values(ChatErrorCode);
...
describe("ChatError", () => {
  it("extends AgentSDKError", () => {
    const err = new ChatError("test", { code: ChatErrorCode.NETWORK });
    expect(err).toBeInstanceOf(AgentSDKError);
    expect(err).toBeInstanceOf(Error);
  });

  it("sets code, retryable, and timestamp", () => {
    const err = new ChatError("msg", { code: ChatErrorCode.NETWORK, retryable: true });
    expect(err.code).toBe(ChatErrorCode.NETWORK);
  ...
  it("defaults retryable to false", () => {
    const err = new ChatError("msg", { code: ChatErrorCode.PROVIDER_ERROR });
  ...
  it("preserves cause", () => {
    ...
    const err = new ChatError("wrapped", { code: ChatErrorCode.NETWORK, cause });
...
describe("ChatSDKError alias", () => {
  it("ChatSDKError is ChatError", () => {
    expect(ChatSDKError).toBe(ChatError);
    const err = new ChatSDKError("test", { code: ChatErrorCode.NETWORK });
...
describe("classifyError", () => {
  it("returns ChatError as-is", () => {
    const err = new ChatError("test", { code: ChatErrorCode.NETWORK, retryable: true });
  ...
  it("classifies timeout messages as TIMEOUT", () => {
    ...
    expect(err.code).toBe(ChatErrorCode.TIMEOUT);
    expect(err.retryable).toBe(true);
  });
  ...
  it("classifies ZodError by name as INVALID_INPUT", () => {
    ...
    expect(err.code).toBe(ChatErrorCode.INVALID_INPUT);
    expect(err.retryable).toBe(false);
  });
  ...
  it("classifies HTTP 401 as AUTH_INVALID", () => {
    ...
    expect(err.code).toBe(ChatErrorCode.AUTH_INVALID);
    expect(err.retryable).toBe(false);
  });
  ...
  it("classifies HTTP 429 as RATE_LIMIT with retryAfter in ms", () => {
    ...
    const err = classifyError(httpErr);
    expect(err.code).toBe(ChatErrorCode.RATE_LIMIT);
    expect(err.retryable).toBe(true);
    expect(err.retryAfter).toBe(60000);
  });
  ...
  it("classifies HTTP 500 as PROVIDER_ERROR", () => {
    ...
    const err = classifyError(httpErr);
    expect(err.code).toBe(ChatErrorCode.PROVIDER_ERROR);
    expect(err.retryable).toBe(true);
  });
  ...
  it("classifies context overflow as CONTEXT_OVERFLOW", () => {
    ...
    expect(err.code).toBe(ChatErrorCode.CONTEXT_OVERFLOW);
    expect(err.retryable).toBe(false);
  });
  ...
  it("handles null/undefined", () => {
    expect(classifyError(null).code).toBe(ChatErrorCode.PROVIDER_ERROR);
    expect(classifyError(undefined).code).toBe(ChatErrorCode.PROVIDER_ERROR);
  });
...
describe("ExponentialBackoffStrategy", () => {
  ...
  it("returns null for non-retryable errors", () => {
    const strategy = new ExponentialBackoffStrategy();
    const err = new ChatError("no", { code: ChatErrorCode.AUTH_INVALID, retryable: false });
...
describe("withRetry", () => {
  ...
  it("retries on retryable error", async () => {
    const networkErr = new ChatError("fail", { code: ChatErrorCode.NETWORK, retryable: true });
    const fn = vi
      .fn()
  ...
  it("throws immediately on non-retryable error", async () => {
    const authErr = new ChatError("denied", { code: ChatErrorCode.AUTH_INVALID, retryable: false });
...
describe("isRetryable", () => {
  it("returns true for retryable ChatError", () => {
    expect(isRetryable(new ChatError("fail", { code: ChatErrorCode.NETWORK, retryable: true }))).toBe(true);
    expect(isRetryable(new ChatError("slow", { code: ChatErrorCode.RATE_LIMIT, retryable: true }))).toBe(true);
    expect(isRetryable(new ChatError("timeout", { code: ChatErrorCode.TIMEOUT, retryable: true }))).toBe(true);
    expect(isRetryable(new ChatError("500", { code: ChatErrorCode.PROVIDER_ERROR, retryable: true }))).toBe(true);
  });

  it("returns false for non-retryable ChatError", () => {
    expect(isRetryable(new ChatError("denied", { code: ChatErrorCode.AUTH_INVALID }))).toBe(false);
    expect(isRetryable(new ChatError("bad", { code: ChatErrorCode.INVALID_INPUT }))).toBe(false);
    expect(isRetryable(new ChatError("overflow", { code: ChatErrorCode.CONTEXT_OVERFLOW }))).toBe(false);
  });
