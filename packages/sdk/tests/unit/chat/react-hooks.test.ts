...
describe("useChat", () => {
  ...
  it("sets error state on non-recoverable error events from stream (Issue #11)", async () => {
    const session = createMockSession();
    vi.mocked(runtime.createSession).mockResolvedValue(session);
    vi.mocked(runtime.send).mockImplementation(() => {
      return (async function* (): AsyncGenerator<ChatEvent> {
        yield { type: "message:start", id: "m1" as ChatId, role: "assistant" };
        yield { type: "message:delta", delta: { type: "text", text: "partial" } };
        yield { type: "error", error: "rate limit exceeded", recoverable: false };
      })();
    });
