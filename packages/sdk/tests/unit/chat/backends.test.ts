...
// ─── getOrCreateAgent options.tools merge (Issue #10) ──────────

describe("getOrCreateAgent merges options.tools", () => {
  let mockAgent: IAgent;
  let mockService: IAgentService;
  ...
  class TestAdapter extends BaseBackendAdapter {
    ...
    protected captureSessionId(): void {}
  }
  ...
  it("passes options.tools to createAgent config", async () => {
    const tools = [
      { name: "myTool", description: "desc", parameters: {} },
    ];
    const adapter = new TestAdapter({
      agentConfig: { systemPrompt: "test", tools: [] },
      agentService: mockService,
    });
    const session = createMockSession();
    await collectEvents(adapter.streamMessage(session, "Hi", { tools: tools as any }));
  ...
  it("does not set tools when options.tools is empty", async () => {
    const adapter = new TestAdapter({
      agentConfig: { systemPrompt: "test", tools: [] },
      agentService: mockService,
    });
    ...
    const calledConfig = (mockService.createAgent as ReturnType<typeof vi.fn>).mock.calls[0][0];
    expect(calledConfig.tools).toEqual([]);
  });

  it("preserves original agentConfig tools when no options.tools", async () => {
    const originalTools = [{ name: "orig", description: "d", parameters: {} }];
    const adapter = new TestAdapter({
      agentConfig: { systemPrompt: "test", tools: originalTools as any },
      agentService: mockService,
    });
    const session = createMockSession();
    await collectEvents(adapter.streamMessage(session, "Hi"));
    ...
    expect(mockService.createAgent).toHaveBeenCalledWith(
      expect.objectContaining({ tools: originalTools }),
    );
  });
});
