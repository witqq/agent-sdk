...
assert(hasTrigger(ci, 'push'), 'CI runs for pushes to master');
assert(hasTrigger(ci, 'workflow_dispatch'), 'CI supports operator dispatch');
...
assert(publish.concurrency?.['cancel-in-progress'] === false, 'publication cannot be cancelled');

const publishJob = requireRecord(publish.jobs?.publish, 'publication job');
assert(publishJob['runs-on'] === 'ubuntu-24.04', 'publication uses GitHub-hosted Linux');
assertStepUses(publishJob, setupNode);
assertNodeVersion(publishJob);
...
function requireSteps(job) {
  if (!Array.isArray(job.steps)) throw new Error('Workflow steps must be an array.');
  return job.steps.map((step) => requireRecord(step, 'workflow step'));
}

function assertStepUses(job, expected) {
  assert(requireSteps(job).some((step) => step.uses === expected), `workflow uses ${expected}`);
}

function assertStepRun(job, expected) {
  assert(requireSteps(job).some((step) => step.run === expected), `workflow runs ${expected}`);
}

function assertNodeVersion(job) {
  const step = requireSteps(job).find((candidate) => candidate.uses === setupNode);
  assert(step?.with?.['node-version'] === '24.20.0', 'workflow pins Node.js 24.20.0');
  assert(step?.with?.['package-manager-cache'] === false, 'release checks disable package caching');
}
