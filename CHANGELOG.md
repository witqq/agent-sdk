...
- Update all compatible workspace and nested dependencies, including npm 12, TypeScript 6, Vitest 5, Vite 8, Astro 7, Starlight 0.42, Vercel AI SDK 7, Copilot SDK 1, and Claude Agent SDK 0.3.
- Replace the unmaintained tsup build with tsdown while preserving all ESM, CommonJS, declaration, CSS, and package-export contracts.
- Remove all npm audit findings across the root, demo frontend, and documentation lockfiles, with exact allowlists for reviewed native install scripts.
- Make trusted publication retry-safe and publish only the verified local GitHub Release asset, then verify the final npm registry digest.

### Fixes

- Correct the public dark-mode error-banner CSS so strict Vite 8/Lightning CSS builds accept the theme.
- Migrate Starlight autogenerated sidebar groups to the current nested `items` schema.
- Remove a duplicate type import exposed by the Vitest 5 parser.
